...
do
  ...
  "${DOCKERBIN}" build "${DOCKERBUILD_OPTS[@]}" \
    -t ${REGISTRY_PREFIX}/govway-installer:${VER:-${LATEST_GOVWAY_RELEASE}}_${arch_suffix} \
    --platform ${platform} \
  ...
  DOCKERBUILD_OPTS=(${DOCKERBUILD_OPTS[@]} '--build-arg' "source_image=${REGISTRY_PREFIX}/govway-installer:${VER:-${LATEST_GOVWAY_RELEASE}}_${arch_suffix}")
...
#    image: ${REGISTRY_PREFIX}/govway:${VER:-${LATEST_GOVWAY_RELEASE}}_batch_${docker_server_platform}
#    #command: Giornaliere
