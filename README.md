...
-v : Imposta la versione dell'installer binario di govway da utilizzare per il build (default :3.3.1)
...
```
./build_standalone.sh -t govway_standalone:3.3.1
docker run -v ~/govway_home:/var/govway -p 8080:8080 -p 8443:8443 govway_standalone:3.3.1
```

In modalità compose

```
./build_compose.sh -t govway_compose:3.3.1
cd target 
docker-compose up
```
...
```
docker run govway_compose:3.3.1 true
