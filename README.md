...
```shell
CONTAINER_ID=$(docker run -d -e GOVWAY_DEFAULT_ENTITY_NAME=Ente -e GOVWAY_DB_TYPE=postgresql linkitaly/govway:3.4.2.p1 initsql);
...
```shell
CONTAINER_ID=$(docker run -d -e GOVWAY_DEFAULT_ENTITY_NAME=Ente -e GOVWAY_DB_TYPE=postgresql -e GOVWAY_DB_MAPPING="T,S" linkitaly/govway:3.4.2.p1 initsql);
docker wait ${CONTAINER_ID};
docker cp ${CONTAINER_ID}:/opt/postgresql .;
docker rm ${CONTAINER_ID}
```
...
.... \
linkitaly/govway:3.4.2.p1_batch giornaliere
```
...
```bash
docker run -e GOVWAY_DB_TYPE=postgresql -v ./custom.properties:/etc/govway_as_jvm.properties:ro linkitaly/govway:3.4.2.p1
```
