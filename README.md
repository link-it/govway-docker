...
- Nel modo keystore/truststore montato (c/d con `GOVWAY_AS_HTTPS_TRUSTSTORE`), se il file **PKCS12** fornito usa l'algoritmo di cifratura moderno (default di OpenSSL 3.x e delle versioni recenti di `keytool`: PBES2/PBKDF2/AES-256), il container **si arresta subito con un errore esplicito** invece di avviarsi: la libreria BouncyCastle inclusa in ogni webapp di GovWay si registra come provider JCE e intercetta la risoluzione di `KeyStore.getInstance("PKCS12")`, e la sua implementazione ha un limite noto e tuttora aperto ([bcgit/bc-java#958](https://github.com/bcgit/bc-java/issues/958), [#1018](https://github.com/bcgit/bc-java/issues/1018)) nel decifrare questo algoritmo — fallirebbe più avanti con un errore fuorviante ("keystore password was incorrect" anche a password corretta). Non è un limite della JVM: un PKCS12 con lo stesso algoritmo si legge correttamente al di fuori di un processo GovWay. Rigenerare il file con `openssl pkcs12 -export -legacy ...` oppure `keytool ... -J-Dkeystore.pkcs12.legacy`. I formati **JKS** non sono affetti da questa limitazione.
